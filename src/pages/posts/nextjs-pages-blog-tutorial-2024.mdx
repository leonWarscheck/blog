...
readingTime: '28min'
preview:
  'The step-by-step guide to how you can build a blog-website just like this -
...
<div className="text-sm text-neutral-400">
  February 09, 2023 - Updated January 2024 - 28min - #blog #nexjs #tutorial
  #webdev #docs
</div>

Welcome to the step-by-step guide of how you can build a blog-website just like
this - using React, Tailwind, NextJS, MDX and MailChimp.
...
To build this setup, I had to pick pieces from many different tutorials. In this
guide I bring it all together.
...
_Disclaimer: The instructions of this guide are just my personal
recommendations. Feel free to customize to your own taste._
...
this blog project _(HTML, CSS, JavaScript, React)_.

For learning _Tailwind_, scan through
[this](https://youtu.be/ft30zcMlFao?si=U1WJGAnrofYRtGtk) tutorial on 1.5-2x
speed. Then learn to actually use it by building your own blog with it. With
Tailwind-Docs-Search and ChatGPT, this is fun and easy. Except for some basic
CSS overview, I had almost zero experience with it before this blog.
...
_tailwind-classes_ that I used in my
[source code](https://github.com/leonWarscheck/blog) on Github.

If you are new to to _Github/_ version control: Work through
[this](https://youtu.be/ulQA5tjJark?si=ZhyJQfluX-rneEbr) Git Crash Course. And
make sure to read through
_[conventional commits](https://www.conventionalcommits.org/en/v1.0.0/)_.

_Accessibility_ is not my strength yet, so I only adress a few of its aspects in
this guide. If you want to go deeper on it, my mentor recommends
[this resource.](https://developer.mozilla.org/en-US/docs/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/React_accessibility)

IMPORTANT: Before starting your blog project, I can also recommend to watch
[this](https://youtube.com/playlist?list=PL4cUxeGkcC9g9gP2onazU5-2M-AzA8eBw&si=twXEDUCdzgd17ovh)
little _NextJS_ course on 1.5-2x speed (no need to code along there imo). This
...
<br />
## <a id="vision-scope" href="#prep-and-setup">
  {' '}
  Vision/ Scope{' '}
</a>
...
Seriously, everything is a remix. Pick whatever sparks your fire, even outside
the box of websites.
...
colortheme, font vibe, most important elements and pages. I used Photoshop, but
Gimp or even pen and paper could do. IMPORTANT: Also draft out how it should
look on mobile aswell!

Knowing precisely what I wanted to build, and seeing it almost alive from the
start, felt really helpful to me. This kept me motivated and always clear about
the next steps.

It is also a good measurement of how closely you can create what you (or your
designers) have planned. Of course you can keep happy accidents, but don't take
them as an excuse to not create and learn what you wanted to in the first place.
...
Brainstorm notes of all the elements, features and effects you want as
bulletpoints.
...
Brainstorm a list of all the major (react-) components and pages you will likely
...
<br />
## <a id="setup" href="#prep-and-setup">
  Setup
</a>

### <a id="installation" href="#prep-and-setup">Installation</a>

- install and run the NextJS project as described in the
...
- delete all unneccessary components of the NextJS default template
  - `git branch empty-template` to save this as a canvas for css experiments
...
<div className="prose-pre:bg-neutral-80 prose-pre:mx prose-pre:leading-4">
  ```md src/ ├── components ├── pages/ │ ├── api │ ├── posts/ │ │ └──
  blog-titles.mdx // auto-routed to .com/posts/filename (+ collected for
  feedData.js utility) │ ├── _app.js │ ├── _document.js │ ├── 404.js //
  auto-routed and hooked up as 404 via reserved filename │ ├── about.js // .js
  (and .mdx!) in pages/ get auto-routed to .com/filename │ └── index.js //
  auto-routed as .com/ via reserved filename ├── styles ├── utils/ │ ├──
  feedData.js // (getStaticProps: fetches blogfeed preview data from
  ./pages/posts to index.js) │ └── ... └── ... ```
</div>

NextJS PR determines the URL path of your different pages by their position in
the folders and a set of special purpose filenames. Make sure to use this exact
folder structure and the rules described in the comments.
...
- test if the routes exist, by typing them in the URL bar of the browser
  - congrats, you now understand the pages-routing basics, let's learn how to
    build on top of that in the main building section

<br />
## <a id="main-building" href="#toc">
  2. Main Building
</a>

In this section I explain most of the smaller steps of the main building
process.

To keep it organized, I wrote the bigger challenges into seperate, detailed
articles. You can find them here:

- <Link href="/posts/mdx-tailwind-rehype-nextjs" className="underline">
    {' '}
    MDX & Tailwind in Next 14 PAGES-routing (incl. Syntax Highlighting)
  </Link>
- <Link href="/posts/mailchimp-nextjs-api" className="underline">
    Mailchimp Newsletter Management & Custom SubscribeForm | Next API
  </Link>
- <Link href="/posts/css-height-sticky-footer" className="underline">
...
<br />
### <a id="html-overview" href="#main-building">
  HTML Overview
</a>

Before we go into the components, there are a few HTML concepts that are helpful
to keep in mind for the project:
...
export default function Footer() {
  return (
    <footer>
      <nav>
        {' '}
        // eventhough navbar is mostly associated with the header, navigation
        links in the footer are also usually a navbar
        <Link href="/">
          <h1>LeonWarscheck</h1>
        </Link>
        <ul>
          <li>
            {' '}
            // instead of "About", I link to the Imprint page (obligation in
            germany)
      ...
      <nav>
        {' '}
        // on smaller screensizes this nav replaces the previous sibling nav,
...
Now that you have `Header` and `Footer`, you can build the NextJS `Layout`
component. It is also pretty simple and self explanatory:
...
export default function Layout({ children }) {
  return (
    <div role="region" aria-label="Main Layout">
      {' '}
      // div for css purpose, otherwise use react fragments
...
export default function MyApp({ Component, pageProps }) {
  return (
    <Layout>
      <Head>
        {' '}
        // set the default browsertab title
        <title>LeonWarscheck</title>
      </Head>
      <Component {...pageProps} /> // this is where the individual pages and
      their props "dock into" the app
    </Layout>
  );
}
```
...
Just like in
[this NextJS docs tutorial](https://nextjs.org/learn-pages-router/basics/create-nextjs-app),
you want the index page to act as a feed with previews of your articles. For
this you need a `HomeFeed` component and a metadata utility component in a
separate file.

First, make sure you write metadata in frontmatter format at the very top of
...
Then finally you import `getFeedData` into your Homepage, fetch the feedData via
`getStaticProps` and populate the feed.
...
export async function getStaticProps() {
  // getStaticProps lets NextJS include the data in the pre-render
...
export default function Home({ feedData }) {
  return (
    <main>
      <ul>
        {' '}
        // a feed is a list. Each .map iteration step creates a new li aka a new
        post preview
        {feedData.map(({ slug, date, readingTime, title, preview }) => (
          <li key={slug}>
            {' '}
            // in React, dynamically created lis need unique identifiers, to be
            managed properly in case of state changes
            <Link href={`/posts/${slug}`}>
              <h2>
                {title} // use jsx to pass the data directly to its target
                destination
              </h2>
              <h3>
            ...
            // wrap the link around the preview, if you want a bigger surface
            than just the title to be clickable
          </li>
        ))}
      </ul>
    </main>
...
need the usual `[id].js` file with `getStaticRoutes` for parsing vanilla
markdown files as pages. That's why the `getFeedData.js` file is also a lot
simpler than in most NextJS tutorials.
...
the `Header` component where the `Menu` is nested and lifting its state to. The
main "trick" here, is the conditional rendering and passing the right props to
the right places:
...
export default function Menu({
  recievedHandleMenuToggle,
  menuOpen,
  handleMenuClose,
}) {
  return (
    <>
      <button onClick={recievedHandleMenuToggle}>
        {' '}
        // Menu show/hide handler prop from header, explained a bit lower.
          ...
          <ul>
            ...
            <li className="relative">
              <form onSubmit={handleSubscribe}>
                {' '}
                // Subscribe mailchimp api logic explained in article mentioned
                earlier.
                <input />
...
Now take a look at the `Header` component again. In the "Header and Footer"
section earlier I left out the `Menu` logic to keep it focused. Here you can see
all of it included and explained:
...
export default function Header({}) {
  ...
  useEffect(() => {
    // The scrollbar of the underlying page shows above the menu, if you don't implement this useEffect to hide it.
  ...
  const handleMenuClose = () => {
    // One-directional(false only) state handler.
    setTimeout(() => {
      // The menu closes faster than the underlying new page loads, so you can experiment with delaying the close.
      setMenuOpen(false);
    }, 0); // In build mode the effect is almost invisible so you can bring it to 0 or leave it out completely.
  };

  const handleMenuToggle = () => {
    // Bi-directional(boolean toggle) state handler. Its used by the main button in the menu component, and needs to be lifted because it is useState-dependent.
  ...
  return (
    <header>
      <nav>
        ...
        <ul>
          {' '}
          // Does not show on smaller screens, Menu component main button shows
          instead.
          <li>
...
In bigger projects this might makes sense because every state change creates a
re-render of both the parent and the child. In this case it is ok though,
because the state only changes when interaction with the menu is happening and
both components are lightweight anyway.

*I called the prop-attribute "recieved" to emphasize that the attribute name can
be chosen freely. *Props are passed through the value assigned to the attribute,
not through the attribute name itself.\* Metaphor: The attribute name is like a
child holding out a named bucket to the parent saying "You can put the prop in
here."
...
> This way you set the similarities for all sizes first and then only add the
> differences via breakpoints.
...
misalignments _in all of the dev tools screen sizes_ over and over again. I
often stumbled upon meaningful styling problems, by testing the edgecases this
way. The sooner you find them the better.
...
it into `/src/styles/fonts`. Then just copy the structure and import from below
and use `${myFont.className}` to insert the font into the CSS of the
`Main Layout <div>`.
...
const myFont = localFont({
  // The localFont function processes the font files and creates a unique classname.
  src: [
    {
...
- a character at the start of each line (to keep the whitespaces before the
  first ascii characters)
- To make it look good with these extra charachters, I added characters all
  around the ascii content, which now serves as a border/ frame.

Tipp: Check out the 404 pages of your reference websites, to get a feel for a
good 404.
...
1. If you click to another page in the meantime, you will get redirected to home
   shortly afterwards.
...
A small but important detail is to set your favicon - the icon of your blog that
gets shown in the browser tab and in search engine results.
...
**Congratulations,** you now know how to build all the foundational components.
Make sure to check out the articles about the rest of this project:
